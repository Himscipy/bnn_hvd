...
flags.DEFINE_string("data_dir",default=os.path.join('./results', "bayesian_neural_network/data/"), help="Directory where data is stored (if using real data).")
...
flags.DEFINE_bool('kl_regularizer',default=True,help='Verbose to print extra details about the model')
flags.DEFINE_float('Alpha',default=1.0,help='Use the scaling for the KL divergence term')
flags.DEFINE_integer('eval_freq',default=100,help='Use the eval frequency to do the MC iteration')
flags.DEFINE_bool('Alpha_State',default=False,help='Set Alpha term on/off')
flags.DEFINE_bool('Restart',default=False,help='Set Alpha term on/off')
...
def main(argv):
    ...
    if hvd.rank() == 0:
        with open( os.path.join(FLAGS.data_dir + "Parsed_Arg"+".log"), 'w') as _out:
    ...
    if hvd.rank() == 0:
        Results_file =  os.path.join(FLAGS.model_dir,'Timing_results_'+str(hvd.size())+'.txt')
        Results_Train_file =  os.path.join(FLAGS.data_dir,'Results_Train_'+str(hvd.size())+'.csv')
    ...
    test_batch_generator = DataObj.train_input_generator(x_test, y_test, batch_size=FLAGS.batch_size)
    ...
    with tf.train.MonitoredTrainingSession(checkpoint_dir=checkpoint_dir,hooks=hooks,config=config) as sess:
        ...
        while not sess.should_stop():
            ...
            if step % FLAGS.print_step == 0 and (not sess.should_stop()) == True  :    
                ...
                with open(FLAGS.data_dir + "SavedWeight_Mean_STD_DATA_"+str(step), "wb") as out:
                    pickle.dump([names,qm_vals, qs_vals], out)
            
                    if HAS_SEABORN:
                        #print ('Evalated qm_vals, qs_vals now Plotting & storing...!',flush=True)
                        fname = FLAGS.data_dir + "SavedPlot_Mean_STD"+str(step)+'.png'
            ...
            if hvd.rank() == 0:
                ...
                if (step + 1) % FLAGS.eval_freq == 0:                
                    ...
                    # train_handle, heldout_handle  = sess.run([training_iterator,heldout_iterator])
                    Test_Image , Test_Label = next(test_batch_generator)
                    ...
                    probs = np.asarray([sess.run((labels_distribution.probs),
                                                feed_dict={images: Test_Image, labels:Test_Label})
                                        for _ in range(FLAGS.num_monte_carlo)])
                    mean_probs = np.mean(probs, axis=0)

                    _, label_vals = sess.run(
                        (images, labels), feed_dict={images: Test_Image, labels:Test_Label})
                    
                    heldout_lp = np.mean(np.log(mean_probs[np.arange(mean_probs.shape[0]),
                                                        np.int32(label_vals).flatten()]))
                    ...
                    for _ in range(20):
                        sess.run(valid_accuracy_update_op, feed_dict={images: Test_Image, labels:Test_Label})
                    
                    valid_value = sess.run(valid_accuracy, feed_dict={images: Test_Image, labels:Test_Label})

                    
                    print( (str(step)+','+str(heldout_lp)+','+str(valid_value)+"\n"), flush=True)
                    

                    sess.run(reset_valid_op)
                # else:
                #     Test_loss_value, Test_accuracy_value = sess.run([loss, Def_accuracy], feed_dict={images: Test_Image,labels:Test_Label})
                #     f_2.write(str(step)+","+str(Test_loss_value)+","+ str(Test_accuracy_value)+"\n")
                #     f_2.flush()
                #     #out_print = "##Step: {:>3d} Loss: {:.3f} Avg_Accuracy: {:.3f} Time: {:.3f}".format(step, loss_value, Test_accuracy_value)
        ...
        with open( os.path.join(FLAGS.data_dir , ("PlotRunTimeIteration_Rank_" + str(hvd.rank())) ), "wb") as out:
