...
#EJ - for single worker we can still run Horovod -shouldn't impact results
...
class Dataset(object):
    def __init__(self, args, SNR=1.0, shift=True, 
...
class Train_Model(tf.keras.Model):
    ...
    def __init__(self):
        ...
        # Bayesian layers
        #
        self.linear1_1 = tfp.layers.DenseFlipout(128,  activation=tf.nn.relu,seed=seed(), name='linear1_1')
        self.linear1_2 = tfp.layers.DenseFlipout(128,  activation=tf.nn.relu,seed=seed(), name='linear1_2')
        self.linear1_3 = tfp.layers.DenseFlipout(128,  activation=tf.nn.relu,seed=seed(), name='linear1_3')

        self.linear2_1 = tfp.layers.DenseFlipout(128, activation=None,seed=seed(), name='linear2_1')
        self.linear2_2 = tfp.layers.DenseFlipout(128, activation=None,seed=seed(), name='linear2_2')
        self.linear2_3 = tfp.layers.DenseFlipout(128, activation=None,seed=seed(), name='linear2_3')

            
        self.final_1 = tfp.layers.DenseFlipout( 1,  activation=tf.nn.tanh,seed=seed(), name='final_1') # or softplus
        self.final_2 = tfp.layers.DenseFlipout( 1,  activation=tf.nn.tanh,seed=seed(), name='final_2') # or softplus
        self.final_3 = tfp.layers.DenseFlipout( 1,  activation=tf.nn.tanh,seed=seed(), name='final_3')
...
class RUN_Model(object):
    def __init__(self,sess_conf,args,model,batch_size, signal_length, num_of_pred_var=3, beta1=1., beta2=1., beta3=1.):
        ...
        if args.hvd:
            import horovod.tensorflow as hvd
            hvd.init()
            #self.learning_rate = hvd.size()*args.lr
    ...
    def train(self,dataset, iterations=1000, print_step=50, dropout_rate=0.10):
        ...
        # Seed input
        self.sample_distribution = self.labels_distribution.sample(seed=seed())
        ...
        #saver_hook = tf.train.CheckpointSaverHook(checkpoint_dir=dataset.final_save_path,save_secs=None,save_steps=100,saver=tf.train.Saver(),checkpoint_basename='model.ckpt',scaffold=None)
        ...
        #bnn_layers = ['linear1_1', 'linear1_2']
        ...
        with tf.train.MonitoredTrainingSession(checkpoint_dir=self.checkpoint_dir,hooks=self.hooks,config=self.sess_conf) as self.sess:
            ...
            while not self.sess.should_stop():
                if self.hvd:
                    ...
                    global_step_val_index = global_step_val * hvd.size() # This important since the data-set generation step has if-else conditions
                ...
                #(train_batch_input, train_batch_output) = dataset.get_batch(int(i), self.batch_size)
                (train_batch_input, train_batch_output) = dataset.train_batch(global_step_val_index,int(i),self.batch_size)
                ...
                if self.hvd:
                    Rank = hvd.rank()
                    if hvd.rank()==0:
                        if (i % print_step == 0): self.print_output=True
                ...
                if self.print_output:
                    ...
                    print('#{},{},{},{},{}'.format(int(i),global_step_val_index,loss_train.mean(),AvRelError,Uncertainty_on_AvRelError))
                ...
                if self.Run_Testdata:
                    ...
                    print('##{},{},{},{},{},'.format(int(i),global_step_val_index,Test_loss_train.mean(),Test_AvRelError,Test_Uncertainty_on_AvRelError))
